...
exports.sendEmailBroadcast = async (req, res) => {
  ...
  try {
    ...
    if (!recipients.length) {
      const hint =
        filters.all || filters.crews.length || filters.roles.length || filters.accessRoles.length || filters.empIds.length
          ? 'No recipients had deliverable email addresses. Sync personnel emails or select "Send to all".'
          : 'Select "Send to all" or at least one crew / access role / recipient filter.';
