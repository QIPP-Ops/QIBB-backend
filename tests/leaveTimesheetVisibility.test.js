...
function mockActor(user = {}) {
  AdminUser.findById.mockImplementation((id) => {
    if (String(id) === '507f1f77bcf86cd799439011') {
      return buildTargetQuery(global.__leaveTimesheetTarget);
    }
    return {
      ...
      if (fields) {
        return {
          lean: jest.fn().mockResolvedValue({
            ...
            name: target.name,
            email: target.email,
            hiddenFromLeaveTimesheet: target.hiddenFromLeaveTimesheet,
          }),
        };
      }
      return query;
...
function mockTarget(overrides = {}) {
  const target = {
    ...
    hiddenFromLeaveTimesheet: false,
    save: jest.fn().mockImplementation(function save() {
      return Promise.resolve(this);
    }),
    ...overrides,
  };
  global.__leaveTimesheetTarget = target;
  mockActor({ email: 'admin@acwaops.com', accessRole: 'admin', role: 'admin' });
  return target;
}

describe('admin leave timesheet visibility routes', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    global.__leaveTimesheetTarget = null;
  });

  test('GET requires super admin', async () => {
    mockTarget();
    const token = tokenFor({ email: 'user@acwapower.com', accessRole: 'viewer', role: 'viewer' });
    const res = await request(app)
      .get('/api/admin/users/507f1f77bcf86cd799439011/leave-timesheet-visibility')
      .set('Authorization', `Bearer ${token}`);
    expect(res.status).toBe(403);
  });

  test('GET returns visibility flag', async () => {
    mockTarget({ hiddenFromLeaveTimesheet: true });
    const token = tokenFor({ email: 'admin@acwaops.com', accessRole: 'admin', role: 'admin' });
    const res = await request(app)
      .get('/api/admin/users/507f1f77bcf86cd799439011/leave-timesheet-visibility')
      .set('Authorization', `Bearer ${token}`);
    expect(res.status).toBe(200);
    expect(res.body.hiddenFromLeaveTimesheet).toBe(true);
  });
  ...
  test('PATCH updates visibility and audits', async () => {
    ...
    const token = tokenFor({ email: 'admin@acwaops.com', accessRole: 'admin', role: 'admin' });
    const res = await request(app)
      .patch('/api/admin/users/507f1f77bcf86cd799439011/leave-timesheet-visibility')
      .set('Authorization', `Bearer ${token}`)
      .send({ hiddenFromLeaveTimesheet: true });
    expect(res.status).toBe(200);
    expect(res.body.hiddenFromLeaveTimesheet).toBe(true);
    expect(target.save).toHaveBeenCalled();
    expect(logAction).toHaveBeenCalledWith(
      expect.objectContaining({ action: AUDIT_ACTIONS.LEAVE_TIMESHEET_VISIBILITY_CHANGED })
    );
  });
