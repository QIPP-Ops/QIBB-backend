...
**Plant ingest (Azure App Settings):** `PLANT_INGEST_MAX_AGE_DAYS=365`, `PLANT_INGEST_INTERVAL_MS=900000` (15 min), `PLANT_INGEST_ON_STARTUP=1`, `AZURE_STORAGE_CONNECTION_STRING` or `BLOB_SAS_URL`, `PLANT_INGEST_MAX_FILES=800`, `TREND_BACKFILL_MAX_DAYS=365`, `BLOB_DOWNLOAD_TIMEOUT_MS=120000`. Blob ingest writes `data/plant-trends-cache.json` (gitignored) and exposes `GET /api/plant-data/trends-cache` for fast frontend loads. RO-HRSG / water filenames are matched case-insensitively; day-column Excel sheets merge into continuous series from January 1. Startup ingest runs asynchronously ~12s after the server listens (`index.js`); it does not block HTTP. If Azure deploys fail with SCM 503/504 during heavy ingest, temporarily set `PLANT_INGEST_ON_STARTUP=0` in App Settings (the scheduled ingest still runs).

**Azure deploy (GitHub Actions):** `.github/workflows/main_qipp-api.yml` builds with `npm ci`, runs tests, reinstalls prod-only deps, zips a lean package (no tests/docs/dev deps/cache), stops the app, deploys via `az webapp deploy` with retries, sets `WEBSITE_RUN_FROM_PACKAGE=1` and `SCM_DO_BUILD_DURING_DEPLOYMENT=false`, then waits on `/health`.
