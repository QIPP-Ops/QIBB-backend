...
const EXTRACT_HEADERS = [
  ...
  'DP at DCS (mbar)',
  'DP from Local (mbar)',
  'GT Inst Air Press (PSIG PANEL) psi',
];

const GT_LABEL_RE = /^GT-\d{2}$/i;
const SKIP_FIRST_CELL_RE = /^max\b/i;
...
function isGtDataRow(firstCellText) {
  const gt = normalizeGtLabel(firstCellText);
  if (!gt || !GT_LABEL_RE.test(gt)) {
    return false;
  }
