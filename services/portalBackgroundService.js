...
// Base64 must stay under MongoDB's ~16MB doc limit.
const BASE64_MONGO_SAFE_BYTES = 12 * 1024 * 1024;
...
function estimateBase64DataUrlBytes(file) {

  const mime = file.mimetype || 'image/jpeg';

  const prefix = `data:${mime};base64,`;

  return Buffer.byteLength(prefix, 'utf8') + Math.ceil(file.buffer.length * 4 / 3);

}



function assertBase64FitsInMongo(file) {

  const estimated = estimateBase64DataUrlBytes(file);

  if (estimated > BASE64_MONGO_SAFE_BYTES) {

    throw Object.assign(

      new Error(

        `Image is too large to store without R2 (${PORTAL_BG_MAX_MB} MB max). Configure R2_PUBLIC_URL for cloud storage.`
...
async function tryUploadPortalBackgroundToR2({ userId, file, mimeType }) {
  ...
  if (!cfg.publicUrl) {
    ...
    // Presigned URLs expire after 7 days — unsuitable for persistent portal backgrounds.
...
async function uploadPortalBackgroundImage({ userId, file }) {
  ...
  if (isR2Configured()) {
    ...
    try {
      ...
      r2Result = await tryUploadPortalBackgroundToR2({ userId, file, mimeType });

    } catch (err) {

      console.error('Portal background R2 upload failed, falling back to base64:', err.message);
