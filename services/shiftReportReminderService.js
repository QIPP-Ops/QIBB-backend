...
async function checkShiftReportsForEndedShift({ dateStr, shiftCode, now = new Date() }) {
  ...
  const reminderOnDuty = [];
  for (const emp of onDuty) {
    if (await isShiftReportEmailRemindersEnabledForCrew(emp.crew)) {
      reminderOnDuty.push(emp);
    }
  }
  if (!reminderOnDuty.length) return { checked: onDuty.length, missing: 0, remindersDisabled: true };

  const crewsOnShift = [...new Set(reminderOnDuty.map((e) => e.crew))];
  const digestLines = [];
  let missingCount = 0;

  for (const crew of crewsOnShift) {
    const crewOnDuty = reminderOnDuty.filter((e) => e.crew === crew);
    const win = shiftWindow(shiftCode, dateStr, crew);
    if (!win || !windowEnded(win, now)) continue;

    const missing = [];
    for (const emp of crewOnDuty) {
  ...
  if (digestLines.length) {
    const label = shiftCode === 'N' ? 'Night' : 'Day';
    await notifyShiftMissing({
      member: null,
      shiftDate: dateStr,
      shiftLabel: label,
      supervisors: [],
      adminDigest: `Missing ${label} reports for ${dateStr}: ${digestLines.join('; ')}`,
    });
  }

  return { checked: onDuty.length, missing: missingCount };
}
