 ...
 *   SEED_FORCE_RESET=1          — wipe AdminUser and AdminConfig first (destructive)
...
async function seedRosterUsers(defaultPassword) {
  ...
  for (const person of rosterData) {
    ...
    if (existing) {
      // Preserve admin edits — only ensure roster accounts stay approved/verified.
      let changed = false;
  ...
  return { created, updated, unchanged, placeholderEmails, total: rosterData.length };
...
async function runAtlasSeed(options = {}) {
  ...
  if (process.env.SEED_FORCE_RESET === '1' || options.forceReset) {
    ...
    log('⚠️  SEED_FORCE_RESET — clearing AdminUser and AdminConfig');
  ...
  log(
    ...
    `👥 Roster: ${roster.created} created, ${roster.updated} updated, ${roster.unchanged} unchanged, ` +
  ...
  return { roster, superAdmin, ptw, rosterCheck };
...
module.exports = { runAtlasSeed, seedRosterUsers };
